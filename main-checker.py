...
def readfile(path,name):
    global c_path
    if os.getcwd()=="/home/pi/Project/finger_print/"+path:
        f=open(name,"rb")
        t=f.read()
    else:
        os.chdir(path)
...
def capture(sgfplib):
    ...
    try:
        ...
        if (result != SGFDxErrorCode.SGFDX_ERROR_NONE):
            ...
            #//////// GET IMAGE QUALITY /login.stud_prof(stud_id,stud_name)//////////#
            cQuality = c_int(0)
...
def match_tmp(sgfplib,temp1,temp2):
    '''print(temp1)
    print("\n")
    print(temp2)'''
    sts=""
...
if __name__=="__main__":
    ...
    try:
        for f in range(f_len):
            ...
            #print(template2)
            m_res=match_tmp(sgfplib,template1,template2)
            #print(m_res)
            m_res=list(m_res)
            #print(m_res[0])
            #print(m_res[1])
